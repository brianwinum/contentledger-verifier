...
Development/build dependencies are pinned in `package-lock.json` and retain their own licenses. They are not application runtime services: Vite and `tsx` are MIT-licensed, TypeScript is Apache-2.0-licensed, and `@types/node` is MIT-licensed. Transitive development dependency notices remain in their distributed packages.
