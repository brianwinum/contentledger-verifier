...
Version `1.0.0` is the production browser verifier for ContentLedger Evidence Bundle v3 packages. The canonical hosted verifier is [verify.wpcontentledger.com](https://verify.wpcontentledger.com/). Every hosted production build records the exact public source revision used to create it. Ordinary local builds identify themselves as local and unpublished. The project remains `private: true` in `package.json` because it is a static application, not an npm package.

The release qualification covers the automated and manual checks documented in this repository. It is not an independent security audit or a publisher signature, and it does not change the verification limits below.
...
Open `http://127.0.0.1:1430/`. A static build is created with `npm run build` in `dist-browser/`; `npm run preview` serves it on `http://127.0.0.1:1431/`. Local builds show a visible local-source notice so they cannot be mistaken for the deployed production release.
...
- the operator types the exact confirmation `deploy production`;
- the full test suite passes;
- two production builds have identical paths, byte lengths, and SHA-256 values; and
- the uploaded static artifact records the exact 40-character source revision and the `production` status.
